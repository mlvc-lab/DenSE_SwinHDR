...
# SSIM
# --------------------------------------------
def calculate_ssim(img1, img2, bit_depth, border=0):
    '''calculate SSIM
    ...
    if img1.ndim == 2:
        ...
        if img1.shape[2] == 3:
            ssims = []
            for i in range(3):
                ssims.append(ssim(img1[:,:,i], img2[:,:,i], bit_depth))
            return np.array(ssims).mean()
        elif img1.shape[2] == 1:
            return ssim(np.squeeze(img1), np.squeeze(img2), bit_depth)
    else:
        raise ValueError('Wrong input image dimensions.')


def ssim(img1, img2, bit_depth):
    C1 = (0.01 * (2 ** bit_depth - 1))**2
    C2 = (0.03 * (2 ** bit_depth - 1))**2
